...
<div class="container">
    <div>
        <form method="get">
            <h1 class="text-center">ค้นหา/แสดงสินค้า</h1>
            <p class="text-center">
            ...
            <tbody>
                ...
                if (isset($_GET['search'])) {
                    if ($_GET['search'][0] == 'T' && is_numeric($_GET['search'][1])) {
                        ...
                        $sql = 'SELECT * FROM category WHERE id LIKE "%' . $_GET['search'] . '%" OR name LIKE "%' . $_GET['search'] . '%" issell = 1 ORDER BY id DESC';
                    }
                } else {
                ...
                if ($result) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo '<button type="button" class="btn btn-primary btn-sm" onclick="loadModal(\'addCart\', ' . $row['id'] .',' .$cus_order . ')">ใส่ตะกร้า</button>' . "\n";
                        echo '</td>' . "\n";
                        echo '</tr>' . "\n";
                    }
                }
