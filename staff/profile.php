<?php
    ...
    $sql = 'SELECT * FROM employee WHERE id = '.$_SESSION['emp_uid'];
    ...
    if (isset($_POST['submit'])) {
        ...
        if (!$name || !$tel || !$email || !$address_no || !$address_district || !$address_amphoe || !$address_province || !$address_zipcode) {
            ...
            if (strlen($email) > 50 || mb_strlen($name) > 50 || strlen($tel) < 9 || strlen($tel) > 10) {
                ...
                $sql = 'UPDATE employee SET name = \''.$name.'\', tel = \''.$tel.'\', email = \''.$email.'\', address_no = \''.$address_no.'\', address_district = \''.$address_district.'\', address_amphoe = \''.$address_amphoe.'\', address_province = \''.$address_province.'\', address_zipcode = \''.$address_zipcode.'\' WHERE id = '.$_SESSION['emp_uid'];
                $result = $conn->query($sql);
                if ($result) {
                    echo '<script>alert("ระบบบันทึกข้อมูลเรียบร้อยแล้ว")</script>';
                }
            }
            if ((isset($_POST['password']) && $_POST['password'] != "") && (isset($_POST['passwordagain']) && $_POST['password'] != "")) {
                $password = $_POST['password'];
                $passwordagain = $_POST['passwordagain'];
                if ($password == $passwordagain) {
                    $sqlpass = md5($password);
                    $sql = 'UPDATE employee SET password = \''.$sqlpass.'\' WHERE id = '.$_SESSION['emp_uid'];
                    $result = $conn->query($sql);
                    if ($result) {
...
?>
<hr><h2 class="text-center">แก้ไขข้อมูลผู้ใช้</h2><hr>
<div class="container">
    <div>
        <form id="profile_staff" method="post" autocomplete="off">
            <table class="table table-borderless text-center">
                ...
                <tr>
                    ...
                    <td width="50%" class="text-left"><input required name="tel" type="tel" placeholder="ตัวเลขไม่เกิน 10 หลัก" value="<?php echo $tel;?>">*</td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td width="50%" class="text-left"><input required name="idth" type="text" minlength="13" maxlength="13" placeholder="เลขบัตรประชาชน 13 หลัก" value="<?php echo $idth;?>" disabled>*</td>
                </tr>
                <th colspan=2></th>
                <tr>
...
<script>
    $.Thailand({
        database: '../js/db.json',
        $district: $('#profile_staff [name="address_district"]'),
        $amphoe: $('#profile_staff [name="address_amphoe"]'),
        $province: $('#profile_staff [name="address_province"]'),
        $zipcode: $('#profile_staff [name="address_zipcode"]'),
    });
</script>
<?php
    require 'footer.php';
