...
$cus_to_order = (isset($_GET['cus'])) && $_GET['cus'] != "" ? $conn->real_escape_string($_GET['cus']) : NULL;
...
if (isset($_GET['clear'])) {
    unset($_SESSION['cart']);
    header("Location: /cart_staff.php", TRUE, 301);
}
...
<div class="container">
    ...
    <div>
        <table class="table table-striped table-bordered text-center">
            ...
            <tbody>
                ...
                if ($cus_to_order) {
                    $cus_order = "?cus=".$cus_to_order;
                } else {
                    $cus_order = "";
                }
               

                echo '<td colspan="8"><button onClick="window.location.href=\'show_product.php' . $cus_order . '\'">เพิ่มสินค้า</button> <button onClick="if(confirm(\'ต้องการล้างตะกร้าใช่หรือไม่?\')) window.location.href=\'cart_staff.php?clear&cus='. $cus_to_order .'\'; else return false;">ล้างตะกร้า</button></td>' . "\n";
                echo '</tr>' . "\n";
                echo '<tr>' . "\n";
    ...
    <?php
    if ($cus_to_order != NULL) {
    ?>
        <div class="row">
            <div class="offset-md-1 col-md-3">
                <label class="control-label"><b>รหัสลูกค้า :</b></label>
                <?= ($cus_to_order != NULL) ? 'C' . sprintf("%05d", $cus_to_order) : "" ?>
            </div>
            <div class="col-md-4">
                <label class="control-label"><b>ชื่อ-นามสกุล :</b></label>
                <?php
                $sql_cus = "SELECT * FROM customer WHERE id = '" . $cus_to_order . "'";
    ...
    <div class="text-center">
        ...
        // } else {
        if ($chq_cart == 0) {
            echo '<button class="btn btn-success" onclick="alert(\'กรุณาเลือกสินค้าที่ต้องการสั่งซื้อ\'); window.location.assign(\'show_product.php?cus='. $cus_to_order .'\');">สั่งซื้อสินค้า</button>';
        } else {
            if ($cus_to_order) {
                $target_link = "orderlist.php?type=addOrder&cus=" . $_GET['cus'];
                echo '<button class="btn btn-success" onclick="loadModal(\'addOrder\', ' . $_GET['cus'] . ')">สั่งซื้อสินค้า</button>';
            } else {
                $target_link = "select_customer.php";
                echo '<button class="btn btn-success" onclick="window.location.assign(\'' . $target_link . '\');">สั่งซื้อสินค้า</button>';
            }
        }
        // }
        ?>
